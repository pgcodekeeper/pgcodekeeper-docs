...
- ⯆ – выпадающее меню с источниками баз данных. Источники баз данных можно выбрать в контекстном меню выбранной БД |source_name| на панели инструментов редактора (2).
...
Поиск происходит по именам объектов. При установленном флажке **RegEx** поиск осуществляется с использованием регулярных выражений. При наличии ``.`` поиск выполняется по квалифицированному имени объекта.
...
Количество строк, получаемых в результатах запроса, можно ограничить в настройке **Limit SELECT rows / Лимит строк SELECT** в глобальных настройках pgCodeKeeper.
