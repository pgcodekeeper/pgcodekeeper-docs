...
Поиск по квалифицированному имени объекта
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Для поиска всех объектов из определенной схемы введите в поле поиска имя схемы с точкой.
...
Для отображения только схемы наберите имя без точки.
