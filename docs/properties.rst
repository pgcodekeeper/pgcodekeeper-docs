...
Библиотеки загружаются в порядке, в котором они указаны в списке (самая верхняя библиотека будет загружена первой). Для изменения порядка необходимо просто перетащить нужную библиотеку в интересуемое место.
...
Диологовое окно создания/редактирования/копирования библеотечной зависимости выглядит следующим образом

.. image:: ../images/lib_props_editor.png

- **Name / Имя** данная библиотека будет отображаться в списке под присвоенным здесь именем, если это поле опущенно то по умолчанию именем будет являться **Path / Путь**.
- **Path / Путь** в этом поле необходимо указать путь до библеотечной зависимости (поддерживается как абсолютный путь так и относительный).
