...
- **Alert if Java heap size less than recommended/Оповещать, если объем heap-памяти Java меньше рекомендуемого** - Будет выводить диалоговое окно при запуске приложения если объем выделенной памяти меньше 2 гигабайт.
