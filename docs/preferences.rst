...
- **Format object code automatically / Форматировать код объектов автоматически** - позволяет отображать отформатированный код функций на языках plpgsql и sql согласно настройкам в глобальных настройках pgCodeKeeper **SQL Editor/Редактор SQL**  и сохранять его в проекте. Подробнее про настройки форматирования кода в :ref:`format`.
- **Limit SELECT rows (0 to disable) / Лимит строк SELECT (0 чтобы выключить)** - позволяет ограничить количество строк, выдаваемых в результате выполнения запроса SELECT. Максимальное количество лимита - 1000 строк, для получения всех результатов необходимо установить 0.
...
- **Migrate data when re-creationg tables / Мигрировать данные при пересоздании таблиц** - опция, позволяющая пересоздавать таблицы с сохранением данных в ней.
Перенос данных осуществляется для таблиц с IDENTITY SEQUENCE колонками.
...
.. image:: ../images/secure_storage.png
