...
Для загрузки списка из файла нажмите по кнопке |import_db| **Import / Импортировать** и выберите необходимый файл. Также вы можете сохранить список в отдельный файл, для этого нажмите по кнопке |export_db| **Export / Экспортировать**. Для добавления типа соединения нажмите на кнопку |add_obj|. Откроется диалог **Create new connection type**

.. image:: .. /images/create_new_connection_type.png

- **Name / Имя** этот параметр будет использоваться в списке данной настройки и в выпадающем меню в диалогового окна **DB credentials** **DB Store / Хранилище БД**.
- **Color / Цвет** этим цветом будут выделены подключения к БД, у которых выставлен тип соединения. Это выделение отрабатывает в списке :ref:`dbStore`, а так же в панели инструментов и SQL редакторе
