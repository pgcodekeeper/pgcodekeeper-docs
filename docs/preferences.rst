...
- **Apply to contents / Приминить к содержимому** – применять правило для совпавшего объекта и всего его содержимого.
...
- **Show code of all child objects when comparing / Показывать код всех дочерних объектов при сравнении** - позволяет отображать в панели сравнения код всех дочерних объектов даже если в них нет различий. По умолчание настройка отключена.
...
Тип соединения
~~~~~~~~~~~~~~
На странице настроек **pgCodeKeeper -> DB Store / Хранилище БД -> Connection type / Тип соединения** находится список записей для типов подключения, который имеет схожий вид и функционал как и в хранилище БД, а диологовое окно отвечающее за создание/редактирование/копирование типов соединений представленно ниже.

.. image:: .. /images/create_new_connection_type.png

При использовании данных настроек подсветка будет работать в следующих случаях: в окне настройки хранилища БД, панели инструментов редактора и при работе в SQL редакторе. В последних двух случаея будет подсвечиваться выпадающее меню(смотрите скрин ниже). Работа данной настройки в хранилище БД наглядно продемонстрированна на скриншоте выше в :ref:`dbStore`.
