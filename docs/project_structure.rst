...
Директория **MIGRATION** содержит сохраненные скрипты миграции. Автоматическое сохранение и удаление скриптов миграции в данной директории можно выбрать на странице настроек :ref:`dbUpdate`.
...
Файл **.dependencies** хранит список библиотек в виде XML-файла. Редактируется через :doc:`настройки проекта <properties>`.

Файл **.pgcodekeeper** является маркером проекта pgCodeKeeper.

Файл **.pgcodekeeperdependencies** содержит неявные зависимости объектов. Редактируется через :doc:`настройки проекта <properties>`.

Файл **.pgcodekeeperignore** содержит правила игнорирования объектов. Подробное описание работы со списками находится в разделе :ref:`ignoreList`. Редактируется через :doc:`настройки проекта <properties>`.

Файл **.pgcodekeeperignoreschema** содержит правила игнорирования схем. Подробное описание работы находится в разделе :ref:`ignoreSchemaList`. Редактируется через :doc:`настройки проекта <properties>`.

Файл **.project** является маркером проекта Eclipse.

Файл **structure.properties** хранит структуру проекта. Редактируется через :doc:`Нормализацию проекта <normalize_project>`.