...
Команда **--mode VERIFY** позволяет переключить работу в режим проверки качества SQL кода, которая позволяет верифицировать код по стилистическим и базовыми правилам. Команды, доступные в этом режиме:
...
.. list-table::
   ...
   * - check_semicolon_after_simple_sql
     - boolean
     - false
     - проверка символа ; после простого sql-выражения
   * - check_space_after_if
     - boolean
     - false
     - проверка наличия пробела после if
   * - check_space_on_math
     - boolean
     - false
     - проверка наличия пробелов до и после математических выражений
   ...
   * - check_temp_table
     - boolean
     - false
     - проверка наличия созданий не temp таблицы
   * - check_indents
     - boolean
     - false
     - проверка отступов (в разработке)
   * - indent_size
     - integer
     - 2
     - значение отступа, используется с check_indents (в разработке)
   * - cyclomatic_complexity
     - integer
     - -1
     - максимально допустимая вложенность кода
   * - max_function_length
     - integer
     - -1
   ...
   * - max_function_params
     - integer
     - -1
     - максимальное количество входных параметров (без учета OUT параметров)
   * - method_count
     - integer
     - -1
...
::
  ...
  ./pgcodekeeper-cli.sh --verify             \
  --ruleset rules.properties                 \
  --sources file1.sql                        \
  --sources file2.sql                        \
  --sources dir
